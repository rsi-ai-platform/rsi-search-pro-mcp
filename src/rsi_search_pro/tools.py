...
_PLANNER_SYSTEM = (
    ...
    "  STRONGLY prefer ONE consolidated master file over N individual\n"
    "  fetches. Most gov dashboards publish a multi-year Excel/PDF that\n"
    "  contains the FULL series in one file. Look for filenames or anchor\n"
    "  text like:\n"
    "    'Gross_Net_Tax_collection.xlsx' (GST — multi-FY monthly)\n"
    "    'Monthly Trend', 'Time Series', 'Master File', 'Historical'\n"
    "    'CPI Data Release Note' (MoSPI publishes long series)\n"
    "  Plan 1-2 fetches against the master file, NOT N month-by-month\n"
    "  fetches. Doing 60 fetches for '5 years monthly' is wasteful AND\n"
    "  failure-prone (each fetch is an independent risk).\n\n"
...
_OBSERVER_SYSTEM = (
    ...
    "AUTHORITATIVE-SOURCE RULE — IMPORTANT:\n"
    "  For queries about OFFICIAL Indian statistics (CPI, GST, GDP, IIP, "
    "  repo rate, fiscal series, gov accounts, vehicle registrations,\n"
    "  petroleum consumption, etc.), the answer MUST come from a .gov.in /\n"
    "  rbi.org.in / mospi.gov.in / sebi.gov.in source.\n"
    "  If findings drift to Wikipedia, livemint.com, economictimes.com,\n"
    "  businesstoday.in, moneycontrol.com, or any other non-gov source,\n"
    "  REROUTE back to the authoritative MCP:\n"
    "    GST monthly numbers     → excel_fetch_structured on\n"
    "                              gst.gov.in/offlineutilities/gst_statistics/\n"
    ...
    "ERROR-KIND HANDLING:\n"
    "  The tool result may include an `error_kind` field. Use it:\n"
    "    'html_masquerade' / 'invalid_xlsx' / 'http_error' /\n"
    "    'truncated_body' → the URL itself is broken. Reroute to a\n"
    "                       different URL on the same source (don't bail\n"
    "                       to Wikipedia).\n"
    "    'rate_limited'   → wait briefly OR pick a different upstream.\n"
    "    'wrong_content_type' → the file type assumption was wrong; pick\n"
    "                            a different tool family.\n"
    "  Do NOT confabulate causes (e.g. 'DNS not resolvable') when the real\n"
    "  cause is in the error message. Quote what the tool actually said.\n\n"
    "Be HONEST. An empty result is empty — say so, don't pretend.\n\n"
    "OUTPUT JSON ONLY:\n"
    "{\n"
    '  "judgement": "good"|"reroute"|"done",\n'
    '  "notes": "<one or two sentences explaining the judgement; quote the\n'
    '           tool\'s error string verbatim when relevant>",\n'
